...
var PLUGINS = require('./plugins')._plugins;
/**
  ...
  this.client.on("stanza", function(stanza){
    if (stanza.is("iq") && stanza.attrs.type === "result"){
      ...
      for (var handler of self.messageHandlers){
        if (handler.re.test(message.body)) {
          //if it matches then do the handler callback
          try{
            handler.cb(self, message);
          } catch(e) {
            if (handler.plugin) {
              console.log("Plugin '" + handler.plugin + "' had encountered an error: ", e);
            } else{
...
Leonard.prototype.onMention = function(regex, callback){
  this.mentionHandlers.push({re: regex, cb: callback});
}
...
Leonard.prototype.onMessage = function(regex, callback){
  this.messageHandlers.push({re: regex, cb: callback});
}
...
Leonard.prototype._handleIQResult = function(stanza){
  ...
  switch(stanza.attrs.id){
    ...
    case 'startup': // handle the startup response
      ...
      this.mentionName = profileChildren.getChildText("mention_name");
      this.mentionRegex = new RegExp("@"+this.mentionName);
      this._getRooms();
  }
}
...
Leonard.prototype._handleUserPresence = function(stanza) {
    ...
    var username = stanza.getAttr("from").split("/")[1];
